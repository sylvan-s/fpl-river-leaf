...
`Live in model` card's own `Rows in model` Trello checklist (see the "Modelling" section on
...
def render_report(review_date, decided, pending, gw, ranked):
    ...
    lines = [
        ...
        "on each `Live in model` Trello card's `Rows in model` checklist, applied by "
    ...
    lines.append(
        ...
        "deferred — open on the Trello board's `Quarantined decisions` list. Shown "
